...
public class UPUtilities {
   public static double[] progressTowardsThreateningSquare(Uberposition u, Move[] m, Uberposition[] u2, int tx, int ty) {
      ...
      for(int k = 0; k < m.length; ++k) {
         ...
         Uberposition u3 = u2[k] == null ? u.evolveWithPlayerMove(0, m[k], -1, -1, 0, 0, 0, 0) : u2[k];
         if (u2[k] != null) {
            u2[k].scrap = false;
         }
         ...
         if (p2 > power) {
            ...
            out[k] = -1.0D;
         } else if (u3.piecesInBetween(m[k].toX, m[k].toY, tx, ty) < u.piecesInBetween(m[k].fromX, m[k].fromY, tx, tx)) {
            out[k] = 0.5D;
         }
      }
