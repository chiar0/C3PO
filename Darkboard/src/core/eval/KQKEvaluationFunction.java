...
public class KQKEvaluationFunction extends EvaluationFunction {
   ...
   public KQKEvaluationFunction() {
      this.addComponent(new KQKMainComponent());
   }

   public String getName() {
      return "KQK Evaluation Function";
   }
   ...
   public boolean canHandleSituation(Metaposition root, int capx, int capy, int check1, int check2, int tries) {
      return root.pawnsLeft + root.piecesLeft == 0 && root.owner.globals.queens == 1 && root.owner.globals.rooks == 0;
   }
   ...
   public Metaposition generateMostLikelyEvolution(Metaposition source, Move m) {
      ...
      this.s2[3] = Metaposition.evolveAfterMove(source, m, 0, -1, -1, 3, 0, 0);
      this.s2[4] = Metaposition.evolveAfterMove(source, m, 0, -1, -1, 4, 0, 0);
